...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void _checkLevelUp() async {
    ...
    if (newLevel > oldLevel && mounted) {
      ...
      setState(() {
        widget.userData?['level'] = newLevel;
      });
    }
  }
  ...
  String get _userBadge =>
      widget.userData?['badge'] as String? ?? 'Rookie Detective 🔍';
                ...
                children: [
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      onPressed: () {
                        // İlk tamamlanmamış modülü bul
                        for (var topic in allTopics) {
                          for (var module in topic.modules) {
                            if (!_progressManager.isModuleCompleted(
                              topic.id,
                              module.number,
                            )) {
                              Navigator.push(
                        ...
                        // Hepsi tamamsa ilk modüle git
                        final firstTopic = allTopics[0];
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => DersDetayScreen(
                              module: firstTopic.modules[0],
                              mainTitle: firstTopic.title,
          ...
          child: Column(
            children: [
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  onPressed: () {
                    // İlk tamamlanmamış konuyu bul (tüm modülleri completed olan quiz'e girilebilir)
                    for (var topic in allTopics) {
                    ...
                    // Hepsi tamamsa ilk quize git
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => QuizStartScreen(
                          quizTopic: allQuizzes[0],
                          username: widget.username,
                        ),
                      ),
                    );
  ...
  List<Widget> get _pages => [
    _buildLessonsScreen(),
    _buildQuizScreen(),
    GameScreen(username: widget.username, userData: widget.userData),
    Center(
